// OSFG Init Test - Tests if optical flow/interpolation initialization breaks display
// This is osfg_demo_simple with optical flow init added (no dispatch)
...
int main() {
    std::cout << "=== OSFG Init Test ===\n";
    std::cout << "Tests if optical flow/interpolation initialization breaks display\n";
    std::cout << "Press ESC to exit\n\n";

    // D3D12
    std::cout << "[1/6] Creating D3D12 device...\n";
    ...
    // Capture
    std::cout << "[2/6] Initializing capture...\n";
    ...
    // Interop
    std::cout << "[3/6] Initializing interop...\n";
    ...
    // Optical Flow (init only, no dispatch)
    std::cout << "[4/6] Initializing optical flow (init only, NO dispatch)...\n";
    ...
    // Interpolation (init only, no dispatch)
    std::cout << "[5/6] Initializing interpolation (init only, NO dispatch)...\n";
    OSFG::FrameInterpolation interpolation;
    OSFG::FrameInterpolationConfig interpConfig;
    interpConfig.width = width;
    interpConfig.height = height;
    interpConfig.format = DXGI_FORMAT_B8G8R8A8_UNORM;  // Match interop format!
    interpConfig.interpolationFactor = 0.5f;
    if (!interpolation.Initialize(device.Get(), interpConfig)) {
        std::cerr << "Failed: " << interpolation.GetLastError() << "\n";
        return 1;
    }

    // Presenter
    std::cout << "[6/6] Creating window...\n";
    ...
    presConfig.windowTitle = L"OSFG Init Test";
    ...
    std::cout << "Running... (optical flow & interpolation are initialized but NOT used)\n";
    std::cout << "If display works, issue is in dispatch. If white, issue is in init.\n\n";
    ...
    while (presenter.IsWindowOpen() && presenter.ProcessMessages()) {
        ...
        // Get texture and present (NO optical flow or interpolation dispatch!)
